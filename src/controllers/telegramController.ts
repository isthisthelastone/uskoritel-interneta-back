...
type FaqAction = { kind: z.infer<typeof faqActionSchema> };
...
function getFaqActionFromCallbackData(data: string | undefined): FaqAction | null {
...
export async function handleTelegramMenuWebhook(req: Request, res: Response): Promise<void> {
  ...
  if (callbackQuery !== undefined) {
    ...
    if (faqAction !== null) {
      const faqText =
        faqAction.kind === "email"
          ? "Вы можете написать нам на почту starlink.echo@outlook.com"
          : [
